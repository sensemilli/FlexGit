...
/**
 * A collision plane is an infinite half-space represented as a plane equation with the form a*x + b*y + c*z + d = 0. Particles are constrained to the positive half space. 
 * Multiple planes can be added to the geometry array to form a convex mesh. 	
 */
...
/**
 * Because the collision geometry is sent to Flex as a flat array of data the geometry types are stored in a generic 16-byte data structure,
...
{
	eFlexShapeFlagTypeMask	= 0x3,		//!< Lower 2 bits holds the type of the collision shape
	eFlexShapeFlagDynamic	= 0x4,		//!< If set this flag indicates the shape is dynamic and should have lower priority over static other collision shapes
 ...
 * @param[in] shapeAabbMins Point to an array of lower AABB coordinates for each shape in world space, should be 4*numShapes in length in x,y,z,* format
...
/**
 * Set the static triangle mesh collision data for the solver, triangles are treated as one-sided with CCW front faces, and collided using a continuous collison detection method to prevent tunnelling
 * 
 * @param[in] s A valid solver
 ...
 * @param[in] cellSize The size of grid cell used for broad phase collision culling, for best performance this should be set relative to the average edge length of the triangle mesh
 * @param[in] lower The lower bound of the triangle positions, should point to an array of 3 floats. If NULL bounds will be re-calculated, if already available the bounds should be provided for efficiency.
 * @param[in] upper The upper bound of the triangle positions, should point to an array of 3 floats. If NULL bounds will be re-calculated, if already available the bounds should be provided for efficiency.
 * @param[in] source The memory space of the source buffer
 */
FLEX_API void flexSetTriangles(FlexSolver* s, const int* indices, const float* vertices, int numTris, int numVertices, float cellSize, const float* lower, const float* upper, FlexMemory source);	
/**
