/*
 * Copyright (c) 2008-2015, NVIDIA CORPORATION.  All rights reserved.
 *
 * NVIDIA CORPORATION and its licensors retain all intellectual property
 * and proprietary rights in and to this software, related documentation
 * and any modifications thereto.  Any use, reproduction, disclosure or
 * distribution of this software and related documentation without an express
 * license agreement from NVIDIA CORPORATION is strictly prohibited.
 */


#ifndef NX_EFFECT_PACKAGE_ASSET_H
#define NX_EFFECT_PACKAGE_ASSET_H

#include "NxApex.h"

namespace physx
{
namespace apex
{

PX_PUSH_PACK_DEFAULT

#define NX_PARTICLES_EFFECT_PACKAGE_AUTHORING_TYPE_NAME "EffectPackageAsset"

/**
Describes an EffectPackageAsset; a collection of particle related effects (emitters, field samplers, etc.)
*/
class NxEffectPackageAsset : public NxApexAsset
{
protected:

	virtual ~NxEffectPackageAsset() {}

public:
	/**
	\brief returns the duration of the effect; accounting for maximum duration of each sub-effect.  A duration of 'zero' means it is an infinite lifetime effect.
	*/
	virtual PxF32 getDuration() const = 0;

	/**
	\brief This method returns true if the authored asset has the 'hint' set to indicate the actors should be created with a unique render volume.
	*/
	virtual bool useUniqueRenderVolume() const = 0;


}; //

/**
Describes an NxEffectPackageAssetAuthoring class; not currently used.  The ParticleEffectTool is used to
author EffectPackageAssets
*/
class NxEffectPackageAssetAuthoring : public NxApexAssetAuthoring
{
protected:
	virtual ~NxEffectPackageAssetAuthoring() {}
};

PX_POP_PACK

} // end of apex namespace
} // end of physx namespace

#endif
