...
#ifdef CONFIGURATION_TYPE_DLL
#define GI_COMPILER_ATTRS __declspec( dllexport )
#else
#define GI_COMPILER_ATTRS __declspec( dllimport )
#endif

#else
// TODO: find appropriate attribute
#define GI_COMPILER_ATTRS
#endif

#define VXGI_FAILED(status) ((status) != ::VXGI::Status::OK)
#define VXGI_SUCCEEDED(status) ((status) == ::VXGI::Status::OK)
#define VXGI_VERSION_STRING "0.9.0.19972074"
...
namespace VXGI
{
    struct Version
    {
        Version() : Major(0), Minor(9), Branch(0), Revision(19972074)
        { }
    ...
    struct DiffuseTracingParameters : public CommonTracingParameters
    {
        ...
        // Enables screen space diffuse tracing correction.
        bool        enableScreenSpaceCorrection;
        ...
        DiffuseTracingParameters()
            ...
            , initialOffsetBias(1.5f)
            , initialOffsetDistanceFactor(1.5f)
            , enableScreenSpaceCorrection(false)
    ...
    struct SpecularTracingParameters : public CommonTracingParameters
    {
        ...
        // When filter == FILTER_NONE, jitter is not used.
        Filter filter;
        ...
        // Enables screen space specular tracing correction.
        bool        enableScreenSpaceCorrection;
    ...
    struct VoxelizationGeometryShaderDesc
    {
        ...
        struct Attribute
        {
            ...
            Attribute() : type(FLOAT_ATTR), width(4), semanticIndex(0), interpolationMode(INTERPOLATION_LINEAR)
            {
    ...
    extern "C"
    {
        ...
        GI_COMPILER_ATTRS Status::Enum VFX_VXGI_CreateGIObject(const GIParameters& params, IGlobalIllumination** ppGI, Version version = Version());

        // Destroys a previously created instance of the VXGI interface object.
        GI_COMPILER_ATTRS void VFX_VXGI_DestroyGIObject(IGlobalIllumination* gi);

        // Creates a shader compiler object for VXGI.
        GI_COMPILER_ATTRS Status::Enum VFX_VXGI_CreateShaderCompiler(const ShaderCompilerParameters& params, IShaderCompiler** ppCompiler, Version version = Version());

        // Destroys a previously created instance of the VXGI interface object.
        GI_COMPILER_ATTRS void VFX_VXGI_DestroyShaderCompiler(IShaderCompiler* compiler);
        ...
        GI_COMPILER_ATTRS Status::Enum VFX_VXGI_VerifyInterfaceVersion(Version version = Version());
        ...
        GI_COMPILER_ATTRS uint64_t VFX_VXGI_GetInternalShaderHash(Version version = Version());

        // Converts a status code to a string containing the name of that status code, e.g. "NULL_ARGUMENT".
        GI_COMPILER_ATTRS const char* VFX_VXGI_StatusToString(Status::Enum status);
    }
} // VXGI
GI_END_PACKING
