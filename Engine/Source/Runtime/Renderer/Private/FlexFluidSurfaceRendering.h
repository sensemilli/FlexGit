...
public:
	 ...
	 */
	void UpdateProxiesAndResources(FRHICommandList& RHICmdList, TArray<FMeshBatchAndRelevance, SceneRenderingAllocator>& DynamicMeshElements);

	/**
	 * Clears the textures, renders particles for depth and thickness, smoothes the depth texture.
	 */
	void RenderParticles(FRHICommandList& RHICmdList, const FViewInfo& View);

	/**
	 * Renders opaque surfaces.
	 */
	void RenderBasePass(FRHICommandList& RHICmdList, const FViewInfo& View);
